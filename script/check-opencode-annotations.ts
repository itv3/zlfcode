...
function dirty() {
  return run("git", ["status", "--porcelain", "--", ...SCOPES]) !== ""
}

const diffBase = dirty() ? run("git", ["merge-base", base, "HEAD"]) : `${base}...HEAD`

function changedFiles() {
  const out = run("git", ["diff", "--name-only", "--diff-filter=AMRT", diffBase, "--", ...SCOPES])
...
function addedLines(file: string): { added: Set<number>; revert: boolean } {
  const diff = run("git", ["diff", "--unified=0", "--diff-filter=AMRT", diffBase, "--", file])
