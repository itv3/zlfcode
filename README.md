...
VS Marketplace / Open VSX 的 `package.json.version` 必须是普通 SemVer，所以市场页面显示 `7.4.109`。GitHub tag、GitHub Release 和 VSIX 文件名使用发布批次 `7.4.1-v0.09`。
...
```bash
git tag zlfcode-v7.4.1-v0.09
git push origin zlfcode-v7.4.1-v0.09
```

发布前必须准备 `.github/release-notes/zlfcode-v7.4.1-v0.09.md`，并确认根 `package.json.version` 与 `packages/kilo-vscode/package.json.version` 都是 `7.4.109`。
...
./node_modules/.bin/vsce package --no-dependencies --skip-license --target darwin-arm64 -o out/zlfcode-7.4.1-v0.09-darwin-arm64.vsix
```
