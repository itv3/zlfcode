...
VS Marketplace / Open VSX 的 `package.json.version` 必须是普通 SemVer，所以市场页面显示 `7.4.508`。GitHub tag、GitHub Release 和 VSIX 文件名使用发布批次 `7.4.5-v0.08`。发布批次 `A.B.C-v0.NN` 必须映射为市场版本 `A.B.CNN`，发布 workflow 会在构建前强制校验该映射、同步包版本、发布说明和当前版本文档。
...
```bash
git tag zlfcode-v7.4.5-v0.08
git push origin zlfcode-v7.4.5-v0.08
```

发布前必须准备 `.github/release-notes/zlfcode-v7.4.5-v0.08.md`，并确认所有同步包版本都是 `7.4.508`。运行以下校验可在打标签前检查版本映射、同步包、发布说明和文档：

```bash
bun run script/zlfcode/check-release.ts zlfcode-v7.4.5-v0.08
```
...
./node_modules/.bin/vsce package --no-dependencies --skip-license --target darwin-arm64 -o out/zlfcode-7.4.5-v0.08-darwin-arm64.vsix
```
