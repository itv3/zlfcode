...
describe("Config", () => {
  ...
  it.effect("migrates v1 provider setup options into AISDK settings", () =>
    Effect.sync(() => {
      ...
      expect(ConfigMigrateV1.migrate({ provider: { empty: { env: null } } }).providers?.empty?.env).toBeUndefined()
      const empty = ConfigMigrateV1.migrate({
        provider: { empty: { models: { empty: { cost: null, limit: null, modalities: null } } } },
      }).providers?.empty?.models?.empty
