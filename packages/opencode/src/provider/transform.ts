...
import { variants as kiloVariants } from "@/kilocode/provider/transform" // kilocode_change - Plus: 下沉自定义 provider 变体逻辑
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  const kilo = kiloVariants(model) // kilocode_change - Plus: 使用下沉的自定义 provider 变体逻辑
  if (kilo) return kilo // kilocode_change - Plus: 使用下沉的自定义 provider 变体逻辑
