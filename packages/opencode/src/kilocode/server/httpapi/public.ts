...
export function matchLegacyKiloOpenApi(input: Record<string, unknown>) {
  ...
  normalizeProviderConfig(spec.components?.schemas?.ProviderConfig)
...
export function normalizeProviderConfig(schema: unknown) {
  if (!schema || typeof schema !== "object") return
  const props = (schema as Schema).properties
  if (props?.env) props.env = nullable(props.env)
  const models = props?.models
  const model = models?.additionalProperties
  if (models && typeof model === "object") models.additionalProperties = nullable(model)
  const fields = typeof model === "object" ? model.properties : undefined
  if (fields?.reasoning) fields.reasoning = nullable(fields.reasoning)
  if (fields?.cost) fields.cost = nullable(fields.cost)
  if (fields?.limit) fields.limit = nullable(fields.limit)
  if (fields?.modalities) fields.modalities = nullable(fields.modalities)
  const map = fields?.variants
  const variants = map?.additionalProperties
  if (variants && typeof variants === "object") {
    variants.additionalProperties = {}
    map.additionalProperties = nullable(variants)
  }
}
