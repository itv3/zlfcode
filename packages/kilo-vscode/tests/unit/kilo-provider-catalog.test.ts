...
describe("KiloProvider catalog refresh", () => {
  ...
  it.skip("publishes only the newest catalog and recommendation after a queued switch", async () => {
  ...
  it.skip.each(["global.disposed", "server.instance.disposed"])(
  ...
  it.skip("global disposal invalidates every view and retries only the new Org while config is delayed", async () => {
    const config = Promise.withResolvers<{ data: Config }>()
    const first = Promise.withResolvers<ReturnType<typeof catalog>>()
    let org = "a"
    let delayed = false
