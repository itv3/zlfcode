...
describe("buildTriggerLabel", () => {
  ...
  it("defaults to the upstream configured semantics when the parameter is omitted (F65)", () => {
    ...
    expect(buildTriggerLabel(undefined, undefined, raw, false, "", true, labels)).toBe("sg / gpt-5.5")
  })
  ...
  it("ignores unresolved raw selection after providers are available (explicit resolved semantics)", () => {
    const raw = { providerID: "sg", modelID: "gpt-5.5" }
    expect(buildTriggerLabel(undefined, undefined, raw, false, "", true, labels, "resolved")).toBe("Select model")
  })
  ...
  it("keeps unresolved raw selection visible with configured semantics", () => {
    const raw = { providerID: "sg", modelID: "gpt-5.5" }
    expect(buildTriggerLabel(undefined, undefined, raw, false, "", true, labels, "configured")).toBe("sg / gpt-5.5")
    expect(buildTriggerLabel(undefined, undefined, raw, true, "Not set", true, labels, "configured")).toBe("sg / gpt-5.5")
  })
  ...
  it("shows only the model id for unresolved kilo raw selection with configured semantics", () => {
    ...
    expect(buildTriggerLabel(undefined, undefined, raw, true, "Not set", true, labels, "configured")).toBe(
      "anthropic/claude-sonnet",
    )
  })

  it("still falls back to clearLabel with configured semantics when nothing is configured", () => {
    expect(buildTriggerLabel(undefined, undefined, null, true, "Not set", true, labels, "configured")).toBe("Not set")
  })

  it("prefers resolved name over raw with configured semantics", () => {
    const raw = { providerID: "anthropic", modelID: "claude-3-5-sonnet" }
    expect(buildTriggerLabel("Claude Sonnet", "anthropic", raw, true, "Not set", true, labels, "configured")).toBe(
      "Claude Sonnet",
    )
  })
