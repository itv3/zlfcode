...
describe("NewWorktreeDialog models", () => {
  it("persists only the saved choice and wires the effective model to display, variants, and guarded submission", () => {
    ...
    // kilocode_change - ZLF：提交前经 validModel 校验（不可用模型不带入），并只统计可用模型分配
    expect(src).toContain("const sel = !isCompare && selected && validModel(selected) ? selected : null")
    expect(src).toContain("return selection.canSubmit(compareMode() ? visibleAllocations() : undefined)")
