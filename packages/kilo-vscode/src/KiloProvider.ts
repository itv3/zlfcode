...
export class KiloProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  ...
  private unsubscribeProvidersChange: (() => void) | null = null
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      switch (message.type) {
        ...
        case "requestProviders":
          this.fetchAndSendProviders(message.mode === "catalog" ? "catalog" : "connected").catch((e) =>
            console.error("[Kilo New] fetchAndSendProviders failed:", e),
          )
          break
        ...
        case "toggleFavorite":
          await this.handleToggleFavorite(message.providerID, message.modelID, message.action)
          break
        case "requestFavorites":
          await this.handleRequestFavorites()
          break
        case "enhancePrompt": {
  ...
  private async doInitializeConnection(): Promise<void> {
    ...
    this.unsubscribeProvidersChange?.()
  ...
  private async refreshConnectionData(): Promise<void> {
    ...
    // 上游 v7.4.21 起 git 状态统一走 refreshGitStatus（内部维护 cachedGitRepo、
    // gitStatus 消息与 stats 轮询的启停），不再在此处手动管理。
  ...
  private favoritesSeeded(): boolean {
    return this.extensionContext?.globalState.get<boolean>(FAVORITES_SEEDED_KEY) === true
  }

  private favorites() {
    return resolveFavorites(this.extensionContext?.globalState.get("favoriteModels"), this.favoritesSeeded())
  }

  private async saveFavorites(favorites: Array<{ providerID: string; modelID: string }>): Promise<void> {
    await this.extensionContext?.globalState.update("favoriteModels", favorites)
    await this.extensionContext?.globalState.update(FAVORITES_SEEDED_KEY, true)
  }

  private async handleToggleFavorite(providerID: string, modelID: string, action: "add" | "remove"): Promise<void> {
  ...
  private async handleRequestFavorites(): Promise<void> {
    const seeded = this.favoritesSeeded()
    const favorites = resolveFavorites(this.extensionContext?.globalState.get("favoriteModels"), seeded)
    if (!seeded) await this.saveFavorites(favorites)
    this.postMessage({ type: "favoritesLoaded", favorites })
  }
