...
export const NewWorktreeDialog: Component<{
  ...
  const visibleAllocations = createMemo(() => {
    ...
    for (const [key, item] of modelAllocations()) {
      if (validModel({ providerID: item.providerID, modelID: item.modelID })) next.set(key, item)
    }
    return next
  })
  ...
  const effectiveVariant = createMemo(() => {
    ...
    // kilocode_change start - ZLF：从未选择过变体（且 agent 配置无默认）时回退模型的
    //「默认推理强度」置顶档——上游 handleSubmit 自 v7.5.6 起把 undefined 兜底为显式
    // 默认（""）传给新会话，若无此回退，配置了置顶档的模型会被锁死为裸发。
    // 显式选择「默认」（stored === ""）不走此回退，保持裸发意图。
    if (stored === undefined) {
      const sel = model()
      const fallback = sel
        ? (provider.findModel(sel) as { defaultVariant?: string } | undefined)?.defaultVariant
        : undefined
  ...
  const canSubmit = () => {
    ...
    if (compareMode() && totalAllocations(visibleAllocations()) === 0) return false
    return true
  }
              ...
              <div class="am-nv-compare-section">
                <div class="am-nv-version-bar">
                  <span class="am-nv-config-label">
                    ...
                    <Show when={totalAllocations(visibleAllocations()) > 0}>
                      <span class="am-nv-compare-count">
                        {totalAllocations(visibleAllocations())}/{MAX_MULTI_VERSIONS}
                      </span>
                    </Show>
                  </span>
                  <button
                  ...
                  trigger={
                    <button class="am-selector-trigger" type="button">
                      <span class="am-selector-left">
                        <Icon name="layers" size="small" />
                        <Show
                          when={visibleAllocations().size > 0}
                          fallback={
                            <span class="am-selector-value am-selector-placeholder">
