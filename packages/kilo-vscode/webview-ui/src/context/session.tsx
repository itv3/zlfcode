 ...
 */
// kilocode_change start: 上游文件本体已逼近 max-lines 上限 3100（v7.6.2 的 goal/通知
// 接线后本体约 3100 行），ZLF 的模型可用性防御与偏好恢复接线净增约 35 行使其超限
//（合并 v7.6.2 时 3135 行）。按 F76 先例（server.tsx 的 complexity 豁免）就地豁免
// 行数规则并保持上游结构，不为行数反复腾挪定制代码——上游后续自身膨胀也不再触发。
...
export const SessionProvider: ParentComponent = (props) => {
  ...
  createEffect(() => {
    ...
    const kilo = pending.modelID ? { providerID: KILO_PROVIDER_ID, modelID: pending.modelID } : undefined
    if (kilo && !validModel(kilo)) {
  ...
  function recoverPrefs(sessionID: string, messages: Message[], names = agentNames()) {
    ...
    if (model && prefs.variant !== undefined) {
