...
export function buildFileAttachments(
  ...
  const dir = workspaceDir ? normalizeAbsolutePath(workspaceDir.replaceAll("\\", "/")).replace(/\/+$/, "") : undefined
  for (const path of mentionedPaths) {
    ...
    const raw = absolute ? path.replaceAll("\\", "/") : `${dir}/${path}`
    const abs = normalizeAbsolutePath(raw)
    if (!isInsideWorkspace(abs, dir)) continue
    const url = new URL("file://")
    // 先转义百分号和空格，避免服务端把文件名中的编码字样或空格解码成其他路径。
